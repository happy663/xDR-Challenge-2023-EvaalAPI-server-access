...
def convert_to_peek_angle_and_compute_displacement_by_gyro(gyro: pd.DataFrame, acc: pd.DataFrame, peaks: np.ndarray, step_length: float, initial_point: dict[str, float], initial_timestamp: float = 0.0):
   # 角速度から歩行タイミングの角度を計算
...
def filter_strongest_beacon_per_interval(df: pd.DataFrame, interval_seconds=10):
    ...
    idx: pd.Series = df.groupby(['interval_index'])[
        'rssi'].transform(max) == df['rssi']
    filtered_df = df[idx]
...
def search_optimal_angle(displacement_df: pd.DataFrame, gt_ref: pd.DataFrame, strong_blescans: pd.DataFrame):
    ...
    for angle in angle_range:
        new_df = rotate_cumulative_displacement(
            displacement_df, angle, {'x': gt_ref.x.iloc[0], 'y': gt_ref.y.iloc[0]})

        # Find nearest rows using merge_asof
        merged_df = pd.merge_asof(new_df.sort_values(
            'ts'), strong_blescans.sort_values('ts'), on='ts', direction='nearest')

        # Calculate Euclidean distances
        merged_df['euclidean_distance'] = np.sqrt(
            (merged_df['x_displacement'] - merged_df['x'])**2 + (merged_df['y_displacement'] - merged_df['y'])**2)
...
def search_optimal_drift_and_step_length(gyro: pd.DataFrame, acc: pd.DataFrame, gt_ref: pd.DataFrame, peaks: np.ndarray):
    ...
    for step_length in step_length_range:
        ...
        for drift in drift_range:
            ...
            cumulative_displacement_df_copy.reset_index(
            ...
            drift_and_euclidean_list.append(
                {"drift": drift, "step_length": step_length, "euclidean_distance": euclidean_distance})
    ...
    for index, rows in cumulative_displacement_df.iterrows():
        ...
        if not is_passable(map_dict, floor_name, nearest_row['x_displacement'], nearest_row['y_displacement'], dx, dy):
            ...
            corrected_point = find_nearest_passable_point(
                map_dict, floor_name, nearest_row['x_displacement'], nearest_row['y_displacement'], dx, dy)
            ...
            delta_x = after_of_correction_point['x'] - \
                before_of_correction_point['x']
            delta_y = after_of_correction_point['y'] - \
                before_of_correction_point['y']

            # 平行移動
            corrected_displacement_df.loc[index:, [
                'x_displacement', 'y_displacement']] += [delta_x, delta_y]
...
def calculate_horizontal_and_vertical_counts(angle_df: pd.DataFrame, rotate_angle: float) -> dict:
    ...
    horizontal_count = len(rotated_angle[
        ((rotated_angle <= 0.1) | (rotated_angle >= 2*np.pi-0.1)) |
        ((rotated_angle >= np.pi - 0.1) & (rotated_angle <= np.pi + 0.1))
    ])
...
def calculate_exist_counts(angle_df: pd.DataFrame, results: pd.DataFrame, gt_ref: pd.DataFrame, edit_map_dict, floor_name, dx, dy):
    for i, row in results.head(20).iterrows():
        rotated_displacement = calculate_cumulative_displacement(
            angle_df.ts, (angle_df['x'] + row['angle']), 0.5, {"x": gt_ref.x[0], "y": gt_ref.y[0]}, gt_ref['%time'][0])

        exist_count = 0
        for _, displacement_row in rotated_displacement.iterrows():
            if is_passable(edit_map_dict, floor_name, displacement_row['x_displacement'], displacement_row['y_displacement'], dx, dy):
                exist_count += 1
...
def get_optimal_angle(results: pd.DataFrame) -> float:
    ...
    optimal_result = results[results['exist_count'] == max_exist_count].sort_values(
        by="horizontal_and_vertical_count", ascending=False).iloc[0]
...
def output_estimate_trajectory_include_ble(
    ...
    cumulative_displacement_df = calculate_cumulative_displacement(
        peek_angle.ts, peek_angle['x'], 0.5, {"x": gt_ref.x[0], "y": gt_ref.y[0]}, gt_ref['%time'][0])
    ...
    first_half_strong_blescans = filter_strong_blescans(
        blescans, center_of_time, -76)
    merged_strong_blescans_first_half = load_beacons_and_merge(
        BEACON_LIST_PATH, first_half_strong_blescans)

    angle = search_optimal_angle(
        cumulative_displacement_df, gt_ref, merged_strong_blescans_first_half)
    ...
    optimal_drift_and_euclidean = search_optimal_drift_from_angle(
        rotate_by_first_half_angle, gt_ref)
    ...
    overall_strong_blescans = filter_strong_blescans(
        blescans, blescans.tail(1)['ts'].values[0], -76)
    merged_strong_blescans_overall = load_beacons_and_merge(
        BEACON_LIST_PATH, overall_strong_blescans)

    overall_strong_ble_angle = search_optimal_angle(
        first_time_remove_drift_angle_displacement, gt_ref, merged_strong_blescans_overall)
    ...
    second_optimal_drift_and_euclidean = search_optimal_drift_from_angle(
        rotate_by_overall_strong_ble_angle, gt_ref)
    ...
    correct_unpassable_displacement = correct_unpassable_points(
        second_time_remove_drift_angle_displacement[second_time_remove_drift_angle_displacement['ts'] < 180], map_dict, floor_name, dx, dy)
...
def output_estimate_trajectory_pdr(
    ...
    rotate_by_first_half_angle_displacement = calculate_cumulative_displacement(
        rotate_initial_direction_angle.ts, rotate_initial_direction_angle['x'], 0.5, {"x": gt_ref.x[0], "y": gt_ref.y[0]}, gt_ref['%time'][0])

    optimal_drift_and_euclidean = search_optimal_drift_from_angle(
        rotate_initial_direction_angle, gt_ref)

    first_time_remove_drift_angle = pd.DataFrame({
        "ts": rotate_initial_direction_angle.ts,
        "x": rotate_initial_direction_angle.x-optimal_drift_and_euclidean['drift']*(rotate_initial_direction_angle.ts-rotate_initial_direction_angle.ts.iloc[0])
    })
    ...
    second_optimal_drift_and_euclidean = search_optimal_drift_from_angle(
        rotate_veritcal_horizontal_angle, gt_ref)

    second_time_remove_drift_angle = pd.DataFrame({
        "ts": rotate_veritcal_horizontal_angle.ts,
        "x": rotate_veritcal_horizontal_angle.x-second_optimal_drift_and_euclidean['drift']*(rotate_veritcal_horizontal_angle.ts-rotate_veritcal_horizontal_angle.ts.iloc[0])
    })

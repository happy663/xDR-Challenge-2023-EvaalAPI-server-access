...
if __name__ == "__main__":
    ...
    created_folder_id = google_drive.create_folder(formatted_now)

    filenames = get_filenames_in_directory("../trials")
    # 結果を表示
    fileter_filenames = [filename for filename in filenames if "pdr" not in filename]
    ...
    for fileter_filename in fileter_filenames:
        ...
        if match is not None:
            extracted_part = re.match(r"(\d+_\d+)_\d+.txt", fileter_filename).groups()[
                0
            ]
